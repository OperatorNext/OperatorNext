...
class BrowserTaskCreate(BaseModel):
    ...
    def validate_task_description(self, v):
        if not v.strip():
...
class Action(BaseModel):
    ...
    args: dict | None = Field(default=None, description="动作参数")
    status: str | None = Field(default=None, description="动作执行状态")
    timestamp: str | None = Field(
        default_factory=lambda: datetime.now().isoformat(), description="动作执行时间"
    )
...
class StepMessage(BaseModel):
    ...
    status: str = Field(
        default="completed", description="步骤状态: pending/running/completed/failed"
    )
    ...
    screenshot: str | None = Field(default=None, description="步骤截图的Base64编码")
    ...
    def __init__(self, **data):
        ...
        # 计算持续时间
        try:
            if isinstance(data.get("started_at"), str) and isinstance(
                data.get("completed_at"), str
            ):
        ...
        if "actions" in data:
            ...
            for action in data["actions"]:
                if isinstance(action, dict):
                    # 确保时间戳是字符串格式
                    if "timestamp" in action and isinstance(
...
class ResultMessage(BaseModel):
    ...
    status: str = Field(
        default="completed", description="任务状态: completed/failed/partial"
    )
    started_at: str = Field(
        default_factory=lambda: datetime.now().isoformat(), description="任务开始时间"
    )
    completed_at: str = Field(
        default_factory=lambda: datetime.now().isoformat(), description="任务完成时间"
    )
    duration: float = Field(default=0.0, description="总执行时长(秒)")
    error_count: int = Field(default=0, description="错误次数")
    retry_count: int = Field(default=0, description="重试次数")
    summary: str | None = Field(default=None, description="任务执行摘要")
    notes: str | None = Field(default=None, description="任务备注")
    metadata: dict | None = Field(default_factory=dict, description="额外的元数据")
...
class ErrorMessage(BaseModel):
    ...
    severity: str = Field(
        default="error", description="错误严重程度: warning/error/critical"
    )
    details: dict | None = Field(default=None, description="错误详情")
    traceback: str | None = Field(default=None, description="错误堆栈")
    step: int | None = Field(default=None, description="发生错误的步骤")
    action: str | None = Field(default=None, description="发生错误的动作")
    timestamp: str = Field(
        default_factory=lambda: datetime.now().isoformat(), description="错误发生时间"
    )
    recoverable: bool = Field(default=True, description="是否可恢复")
    retry_count: int = Field(default=0, description="重试次数")
    notes: str | None = Field(default=None, description="错误备注")
...
class WSMessage(BaseModel):
    ...
    sequence: int | None = Field(default=None, description="消息序号")
    metadata: dict | None = Field(default_factory=dict, description="额外的元数据")
    ...
    def __init__(self, **data):
        ...
        if isinstance(log_data, dict):
            if "screenshot" in log_data:
                log_data["screenshot"] = (
                    f"{log_data['screenshot'][:50]}... (base64数据已截断)"
                )
            elif (
                isinstance(log_data.get("data"), dict)
                and "screenshot" in log_data["data"]
            ):
                log_data["data"]["screenshot"] = (
                    f"{log_data['data']['screenshot'][:50]}... (base64数据已截断)"
                )
    ...
    @validator("type")
    def validate_message_type(self, v):
